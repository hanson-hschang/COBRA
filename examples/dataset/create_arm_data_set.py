...
from utils import sigma_to_shear, pos_dir_to_input
...
idx_list = np.random.randint(len(true_kappa), size=10) # [i*250 for i in range(10)]
fig = plt.figure(1)
ax = fig.add_subplot(111, projection='3d')
fig2, axes = plt.subplots(ncols=3, nrows=2, sharex=True, figsize=(16, 5))
for ii in range(len(idx_list)):
	i = idx_list[ii]
	ax.plot(true_pos[i,0,:], true_pos[i,1,:], true_pos[i,2,:], ls='-', color=color[ii])
