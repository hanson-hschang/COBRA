...
class BR2Environment(BaseEnvironment):
    ...
    def setup_BR2(
        ...
        rest_length = 0.288  # rest length of the BR2 arm (used to be 0.16)
...
def main(
    ...
    if BSR_AVAILABLE:
        ...
        pose_obj = Pose(
            ...
            directors=env.rod.director_collection[:, :, 0].T,
            unit_length=0.03,
        )
        pose_obj.update_material(
            color=np.array([64, 64, 64, 256], dtype=np.float64) / 256.0
        )
